    ...
    def reset(mock_cassandra=False):
        ...
        if mock_cassandra:
            ...
            try:
                ...
                singleton.session.execute(
                    'CREATE TABLE IF NOT EXISTS hecuba.blocks (blockid text, class_name text, ' +
                    'storageobj_classname text, tkns list<frozen <tuple<bigint, bigint>>>, entry_point text, ' +
                    'port int, ksp text, tab text, object_id text, obj_type text, indexed_args list<text>, ' +
                    'nonindexed_args list<text>, key_list list<text>, value_list list<text>, mem_filter text, ' +
                    'PRIMARY KEY(blockid))')
                singleton.session.execute(
