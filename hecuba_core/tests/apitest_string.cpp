...
int main() {
    ...
    s.loadDataModel("model_string.yaml","model_string.py");

    std::cout<< "+ Data Model model_string.yaml loaded"<<std::endl;
    ...
    std::cout<< "+ 'midict' object created"<<std::endl;
    ...
    value = (char*)malloc(strlen("hola mundo")+1);
    memcpy(value, "hola mundo", strlen("hola mundo")+1);

    // Add the numpy to the dictionary: midict[key] = mi_sn
    int ts = 42;
    midict->setItem( &ts, &value);
    std::cout<< "Modified position "<< ts<< " with value " << value<<std::endl;
