...
std::set<int32_t> ArrayDataStore::get_clusters_id(std::set<int32_t> &clusters) {
    std::set<int32_t> one_cluster = {};
    uint32_t cluster_size = ((unsigned)CLUSTER_SIZE << 1u);
    int32_t last_cluster_id = -cluster_size-1;
    std::set<int32_t, std::less<int32_t>, std::allocator<int32_t>>::iterator it = clusters.begin(), itup;
    while (it != clusters.end()) {
        itup = clusters.upper_bound(last_cluster_id + cluster_size);
        if (itup != clusters.end()) {
            last_cluster_id = *itup;
            it = itup;
            one_cluster.insert(last_cluster_id);
        } else break;
    }
    return one_cluster;
}
/***
 * Reads a numpy ndarray using the parameter coord, otherwise it reads the entire numpy
 * @param storage_id of the array to retrieve, metadata of the numpy, coord: coordinates to retrieve the numpy,
 * save: pointer where to save the numpy
...
void *ArrayDataStore::read_n_coord(const uint64_t *storage_id, ArrayMetadata *metadata,
    ...
    if(coord_size != 0) clusters = get_clusters_id(clusters);

    std::set<int32_t, std::less<int32_t>, std::allocator<int32_t>>::iterator it = clusters.begin();
