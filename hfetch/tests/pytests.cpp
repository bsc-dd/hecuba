...
EXPECT_EQ(ok,
1);
EXPECT_EQ(memcmp(data_a, data_b, data_size),
0);
std::free(data_a);
free(data_b);
PY_ERR_CHECK
...
PyList_SetItem(list,
0, Py_BuildValue("i", key1));
PyList_SetItem(list,
1, Py_BuildValue("f", key2));
...
EXPECT_EQ(PyList_Size(result), colsnames
.
...
for (
int i = 0;
i< PyList_Size(result);
++i) {
EXPECT_FALSE(Py_None
==
...
PyObject *list = PyList_New(2);
PyList_SetItem(list,
0, Py_BuildValue("i", 123));
PyList_SetItem(list,
1, Py_BuildValue("f", f1));
float f2 = 5430;
PyObject *list2 = PyList_New(2);
PyList_SetItem(list2,
0, Py_BuildValue("i", 543));
PyList_SetItem(list2,
1, Py_BuildValue("f", f2));
float f3 = 3230;
PyObject *list3 = PyList_New(2);
PyList_SetItem(list3,
0, Py_BuildValue("i", 323));
PyList_SetItem(list3,
1, Py_BuildValue("f", f3));
...
PyList_SetItem(key,
0, Py_BuildValue("i", it));
PyList_SetItem(key,
1, Py_BuildValue("f", fl));
PY_ERR_CHECK
WriteTable->
put_row(key, result
);
...
cass_cluster_set_port(test_cluster,
9042);
...
std::vector <std::string> keysnames = {"partid", "time"};
std::vector <std::vector<std::string>> colsnames = {std::vector < std::string > {"x"},
...
std::vector <std::vector<std::string>> colsnames = {
        std::vector < std::string > {"image", "double", "2x2", "partition", "arrays_aux"}};
std::string token_pred = "WHERE token(partid)>=? AND token(partid)<?";
std::vector <std::pair<int64_t, int64_t>> tokens = {std::pair<int64_t, int64_t>(-10000, 10000)};
...
PyList_SetItem(rw_list,
0,py_array);
T->
put_row(list, rw_list
);
