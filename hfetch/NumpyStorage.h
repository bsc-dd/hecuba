...
public:
    ...
    ArrayMetadata store(std::string table, std::string keyspace, std::string attr_name, CassUuid &storage_id, PyArrayObject* numpy);

    PyObject* read(std::string table, TupleRow* keys, ArrayMetadata &np);

private:

    ArrayMetadata get_np_metadata(PyArrayObject* numpy);
