...
#ifndef Foundation_LRUPersistentStrategy_INCLUDED
#define Foundation_LRUPersistentStrategy_INCLUDED
...
namespace Poco {
    ...
    class LRUPersistentStrategy : public AbstractStrategy<TKey, TValue>
        ...
        void onReplace(const void *, std::set<TKey> &elemsToRemove) {
            ...
            while (i++ < diff) {
                if (it->use_count() == 1) {
                    ...
                    IndexIterator it_v = _keyIndex.find(*it);

                    if (it_v != _keyIndex.end() && it_v->second->use_count() == 1) {
...
#endif // Foundation_LRUPersistentStrategy_INCLUDED
