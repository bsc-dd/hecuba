...
class StorageDict_Tests(unittest.TestCase):
    ...
    def test_iadd(self):
        pd = StorageDict('ksp', 'tb1', False, [('pk1', 'int')], [('val1', 'int')])
        pd.types['pk1'] = "notCounter"
        pd[3] = 0
        pd[3] += 2
        self.assertEqual(2, pd[3])
        self.assertNotEquals(1, pd[3])
        pd2 = StorageDict('ksp', 'tb1', False, [('pk1', 'int')], [('pk2', 'counter')])
        pd2[3] = 3
        pd2[3] += 2
        self.assertEqual(2, pd[3])
    ...
    def inmemory_contains_test(self):
        pd = StorageDict('ksp', 'tb1', False, [('pk1', 'int')], [('val1', 'str')])
        pd[3] = '4'
    ...
    def preparequery_test(self):
        ...
        pd = StorageDict('kksp', 'tt', True, [('pk1', 'int')], [('val1', 'str')])
